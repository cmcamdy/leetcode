...
public:
    ...
    {
        books.insert(books.begin(), vector<int>(2, 0));
        vector<int> f(books.size(), 0);
        f[0] = books[0][1];
        int temp_width;
        int temp_max;
        ...
        {
            f[i] = f[i - 1] + books[i][1];
            temp_width = books[i][0];
            temp_max = books[i][1];
            for (int j = i - 1; j >= 1; j--)
            {
                if (books[j][0] + temp_width <= shelf_width)
                {
                    temp_max = max(temp_max, books[j][1]);
                    f[i] = min(f[j - 1] + temp_max, f[i]);
...
int main(int argc, char const *argv[])
{
    ...
    int res = s.minHeightShelves(books, 200);
    // int res = s.minHeightShelves(books,200);
    cout << res;
    return 0;
}